...
def process_lab_file(path, labName):
    ...
    count = 1
    metaFilePath = parentDirectory + "/" + labName + "_testreport_" + time.strftime('%c') + ".org"
    ...
    for expIndex in range(number_of_experiments):
        ...
        testCasesPath = process_experiment(experiment, directory)
        count = save_link_to_test_cases(metaFilePointer, count, experiment.name, testCasesPath)
...
#+begin_src python :tangle exporter.py :eval no
def save_link_to_test_cases(filepointer, count, expName, testCasesPath):
    for path in testCasesPath:
        basename = os.path.basename(path)
        line = str(count) + ". " + expName + " " + "[[" + path + "]" + "[" + basename + "]]" + "\n"
